...
랜덤으로 구현하고 ajax통신 없이 뷰에서 보여줄 수 있게 구현

### 구현 계획

* 모바일 뷰 구현

width값으로 모바일 뷰를 구현, 글씨 크기나 콘텐츠 크기가 자동으로 조절될 수 있게 부트스트랩 사용. 


### 이슈사항

* $(window).height값이 $(document).height값이랑 똑같이 나옴. 

스크롤을 통해 게시글을 더 가져올 수 있게 하려고 하였으나 window의 height값이 원하는 대로 나오지 않아 구현하지 못하였음. css문제로 추정. 
